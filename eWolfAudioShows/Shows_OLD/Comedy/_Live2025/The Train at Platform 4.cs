...
public class TheTrainatPlatform4 : AudioBaseDetailsOLD
{
    public TheTrainatPlatform4()
    {
        ...
        Shows.Add("Series  1");
        Shows.ShowItems[0].AddLink("XbIOzPjFgUo");
        Shows.ShowItems[0].AddEpisode(1, "Coffee", "It's day one of a new franchise and train manager Sam has high hopes for their maiden voyage, even if the new bosses insist her role is to 'enhance the onboard travel experience'. But the marketing team's offer of free coffee for ALL customers soon threatens to derail the smooth running of the service. Meanwhile passengers struggle to escape the clutches of the automatic toilets...");
        Shows.ShowItems[0].AddEpisode(2, "Cat", "It's Friday night and everyone's looking forward to their weekend plans. But the journey home is struck with disaster when a celebrity cat goes missing on the train - Artemis Montmorency Wolfhandle the Third is potentially the new face of Kitty Krunch Cat Food but is set to miss a crucial audition if the train crew can't track her down. Meanwhile, Dev's plans for a hot date are thrown into turmoil when he learns the bitter truth about his receding hairline.");
        Shows.ShowItems[0].AddEpisode(3, "Bag", "Sam and the rest of the crew struggle to contain an anarchic party of seditious school children. But Dev sees a golden business opportunity in profiting from the kids' snack money. Meanwhile a bag falls from the luggage rack, injuring a passenger, who turns out to work for the rail regulator. Sam struggles to appease him, in order to avoid a damning report.");
        Shows.ShowItems[0].AddEpisode(4, "Gilbert", "When First Class Steward Gilbert discovers his step-sister is a passenger on the train, he's forced to spend the journey hiding from her. It seems that his whole life Gilbert has been living a lie. Meanwhile, Sam must contend with a carriage full of Roundheads on their way to a re-enactment of the Battle of Naseby, armed with a collection of very realistic pikes.");

        Shows.Add("Series  2");
        Shows.ShowItems[1].AddLink("OKsfb_y4J6Q");
        Shows.ShowItems[1].AddEpisode(1, "See It, Say It, Sorted", "Sam is tired of having to ‘sort it’ when passengers ‘see it’ and ‘say it’, but when an actual suspect package is found, the team leap into action.");
        Shows.ShowItems[1].AddEpisode(2, "Get a Room", "Sam befriends a couple who extol the virtues of early retirement, while a younger couple get a little too friendly with each other in first class.");
        Shows.ShowItems[1].AddEpisode(3, "Is This Your Seat?", "A man refuses to move seat, causing a moral crisis on board.   When two American tourists attempt to celebrate their 50th wedding anniversary with a Great British railway journey to York, Sam must negotiate with the bloke in their reserved seat who’s refusing to move.");
        Shows.ShowItems[1].AddEpisode(4, "Smart Train", "Dodgy air conditioning, indecipherable announcements, toilet doors that open randomly – it sounds like your average train journey. But when an update goes wrong, the train’s automated systems cause chaos and things are even worse than normal.  Sam and the team attempt to wrestle back control of a train gone haywire.");
    }
}
